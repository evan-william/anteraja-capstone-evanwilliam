...
import { SellerTicketForm } from '@/components/tracking/seller-ticket-form';
import type { RiskStatus } from '@/lib/tracking/types';

const riskLabel: Record<RiskStatus, string> = {
  on_track: 'Sesuai jadwal', at_risk: 'Berisiko terlambat',
  action_required: 'Perlu tindakan', resolved: 'Sudah ditangani',
};
...
export default async function SellerShipmentDetail({ params }: PageProps) {
  ...
  const journey = buildJourney(shipment);
  ...
  return <><SiteHeader userName={user.name || user.email} role={user.role} /><main id="main-content" className="app-main max-w-5xl"><Button variant="ghost" size="sm" asChild><Link href="/pengiriman"><ArrowLeft /> Kembali ke pengiriman</Link></Button>
    ...
    </section>{journey ? <JourneyMapDisclosure journey={journey} /> : null}<SellerTicketForm awb={shipment.tracking_number} /></main></>;
}
