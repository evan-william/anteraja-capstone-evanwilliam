...
import { accessCodeSchema, awbSchema, issueMessage, ticketSchema } from '@/lib/tracking/validation';
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  const { data, error } = await supabase.rpc('create_tracking_ticket', {
    p_awb: awb.data, p_access_code: code.data, p_note: parsed.data.note,
  });
