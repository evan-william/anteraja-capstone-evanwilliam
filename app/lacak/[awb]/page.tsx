...
export default async function TrackingDetailPage({ params, searchParams }: PageProps) {
  const [{ awb }, { code = '' }, user] = await Promise.all([params, searchParams, getCurrentUser()]);
  return <><PublicHeader /><main id="main-content" className="min-h-screen bg-[#f8f7f5] app-main max-w-[1060px]"><TrackingExperience awb={decodeURIComponent(awb).toUpperCase()} code={code} viewerRole={user?.role ?? null} /></main></>;
}
