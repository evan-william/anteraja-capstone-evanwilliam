...
function SettlementTable() {
  return <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="bg-[#faf8f7] text-[11px] uppercase tracking-wider text-[#7e7270]"><tr><th className="px-5 py-3">Referensi</th><th className="px-4 py-3">Keterangan</th><th className="px-4 py-3">Nominal</th><th className="px-4 py-3">Status</th></tr></thead><tbody>{settlements.map((row) => <tr key={row.awb} className="border-t border-[#eee7e5]"><td className="px-5 py-4"><span className="font-medium">{row.awb}</span><span className="mt-1 block text-xs text-[#8a7d7b]">Resi pengiriman</span></td><td className="px-4 py-4">{row.desc}</td><td className="px-4 py-4 font-semibold tabular">{row.type === 'Keluar' ? '−' : '+'}{money.format(row.amount)}</td><td className="px-4 py-4"><StatusBadge status={row.status} /></td></tr>)}</tbody></table></div>;
}

function StatusBadge({ status }: { status: string }) { const cls = status === 'Cocok' ? 'bg-[#e6f5ed] text-[#287050]' : status === 'Baru' ? 'bg-[#fff1dc] text-[#956017]' : 'bg-[#fbe7e9] text-[#a51f31]'; return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${cls}`}>{status}</span>; }
...
function Info({ label, value }: { label: string; value: string }) { return <div className="rounded-lg bg-[#f7f3f2] p-4"><p className="text-xs text-[#857875]">{label}</p><p className="mt-1 text-sm font-semibold">{value}</p></div>; }
...
function PreviewScreen() {
  return <>
    ...
    <div className="grid gap-3 sm:grid-cols-3"><Summary label="Baru" value="21" sub="perlu kategori" tone="amber" /><Summary label="Cocok" value="126" sub="transaksi lama aman" tone="green" /><Summary label="Error" value="7" sub="diabaikan saat simpan" tone="red" /></div>
    <div className="panel mt-5 overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[940px] text-left text-sm"><thead className="bg-[#f6f2f1] text-[11px] uppercase tracking-wider text-[#756967]"><tr><th className="px-4 py-3">Tanggal</th><th className="px-4 py-3">Keterangan bank</th><th className="px-4 py-3">Referensi pengiriman</th><th className="px-4 py-3">Nominal</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Kategori</th></tr></thead><tbody>{settlements.map((row, i) => <tr key={row.awb} className="border-t border-[#eee7e5]"><td className="px-4 py-4">{18 - i} Sep 2026</td><td className="px-4 py-4"><strong className="font-medium">{row.desc}</strong><span className="mt-1 block text-xs text-[#887b79]">{i % 2 ? 'BANKA-SETTLE-09' : 'TRANSFER ANTERAJA'}</span></td><td className="px-4 py-4 font-medium">{row.awb}</td><td className="px-4 py-4 font-semibold tabular">{money.format(row.amount)}</td><td className="px-4 py-4"><StatusBadge status={row.status} /></td><td className="px-4 py-4"><span className="inline-flex min-w-32 items-center justify-between rounded-lg border bg-white px-3 py-2 text-xs">{i === 0 ? 'Pendapatan COD' : i === 1 ? 'Ongkir' : i === 2 ? 'Retur' : 'Pencairan'}<ChevronRight className="size-3" /></span></td></tr>)}</tbody></table></div><div className="flex flex-col items-start justify-between gap-3 border-t bg-[#fffdfc] p-4 sm:flex-row sm:items-center"><p className="text-xs text-[#746866]"><strong className="text-[#2e2726]">21 baris baru</strong> wajib memiliki kategori sebelum disimpan.</p><PrimaryButton><Check className="size-4" /> Simpan hasil rekonsiliasi</PrimaryButton></div></div>
  </>;
}

function Summary({ label, value, sub, tone }: { label: string; value: string; sub: string; tone: string }) { const map: Record<string,string> = { amber: 'bg-[#fff2de] text-[#9b6517]', green: 'bg-[#e8f6ef] text-[#287050]', red: 'bg-[#fbe8ea] text-[#a92233]' }; return <div className="panel flex items-center gap-4 p-4"><span className={`grid size-10 place-items-center rounded-lg font-bold ${map[tone]}`}>{value}</span><div><p className="text-sm font-semibold">{label}</p><p className="text-xs text-[#817472]">{sub}</p></div></div>; }
...
function HistoryScreen() {
  ...
  return <><PageIntro eyebrow="Audit dan pemulihan" title="Riwayat rekonsiliasi" copy="Lihat hasil setiap impor dan batalkan transaksi yang dibuat oleh impor tanpa menghapus transaksi lama yang hanya dicocokkan." action={<PrimaryButton><Upload className="size-4" /> Rekonsiliasi baru</PrimaryButton>} /><div className="panel overflow-hidden"><div className="border-b p-5"><h2 className="font-semibold">September 2026</h2><p className="mt-1 text-xs text-[#7d706e]">3 proses rekonsiliasi</p></div>{history.map((row, i) => <div key={row[0]} className="flex flex-col gap-4 border-b border-[#eee7e5] p-5 last:border-0 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-4"><span className={`grid size-10 shrink-0 place-items-center rounded-lg ${i === 2 ? 'bg-[#eeeae9] text-[#6e6361]' : 'bg-[#e9f6ef] text-[#2f7d5b]'}`}>{i === 2 ? <RotateCcw className="size-5" /> : <CheckCircle2 className="size-5" />}</span><div><p className="font-semibold">{row[0]}</p><p className="mt-1 text-xs text-[#7d706e]">{row[1]} · {row[2]}</p></div></div><div className="flex items-center gap-3"><StatusBadge status={row[3] === 'Selesai' ? 'Cocok' : 'Perlu dicek'} /><button className="text-sm font-semibold text-[#b72032]">{i === 2 ? 'Lihat detail' : 'Batalkan impor'}</button></div></div>)}</div><div className="mt-4 rounded-lg border border-[#edc7cc] bg-[#fff5f6] p-4 text-sm text-[#76202c]"><strong>Pembatalan aman:</strong> hanya transaksi baru yang dibuat dalam proses impor dipindahkan ke status terhapus. Baris yang sebelumnya cocok tidak berubah.</div></>;
}

function ShipmentScreen() {
  return <><PageIntro eyebrow="Jejak settlement" title="Rincian pengiriman 100078421945" copy="Satu tampilan untuk menelusuri hubungan antara resi, nilai COD, potongan biaya, dan mutasi rekening." action={<SecondaryButton><History className="size-4" /> Lihat riwayat</SecondaryButton>} /><div className="grid gap-5 lg:grid-cols-[1fr_.72fr]"><div className="panel p-6"><div className="flex items-center justify-between border-b pb-5"><div><p className="text-xs text-[#7c6f6d]">Status pengiriman</p><p className="mt-1 flex items-center gap-2 font-semibold"><CheckCircle2 className="size-4 text-[#2e825d]" /> Diterima penerima</p></div><span className="rounded-full bg-[#e9f6ef] px-3 py-1 text-xs font-semibold text-[#287050]">Settlement cocok</span></div><div className="grid gap-5 py-6 sm:grid-cols-2"><Detail label="Nomor resi" value="100078421945" /><Detail label="Tanggal diterima" value="17 Sep 2026, 16.42" /><Detail label="Nilai COD" value="Rp2.800.000" /><Detail label="Biaya layanan" value="Rp55.000" /></div><div className="rounded-xl bg-[#f6f2f1] p-5"><div className="flex items-center justify-between"><span className="text-sm text-[#6f6361]">Settlement bersih</span><strong className="text-xl tabular">Rp2.745.000</strong></div><div className="mt-3 flex items-center justify-between border-t border-[#e2d9d7] pt-3 text-xs"><span className="text-[#7b6e6c]">Mutasi 18 Sep 2026 · BANKA-SETTLE-09</span><span className="font-semibold text-[#287050]">Cocok otomatis</span></div></div></div><div className="panel p-6"><h2 className="font-semibold">Alur dana</h2><div className="mt-6 space-y-0"><Timeline icon={<Truck />} title="Paket diterima" meta="17 Sep · 16.42" done /><Timeline icon={<Banknote />} title="Settlement diproses" meta="18 Sep · 08.10" done /><Timeline icon={<ArrowDownRight />} title="Dana masuk rekening" meta="18 Sep · 14.31" done /><Timeline icon={<Check />} title="Mutasi direkonsiliasi" meta="18 Sep · 14.32" done last /></div></div></div></>;
}

function Detail({ label, value }: { label: string; value: string }) { return <div><p className="text-xs text-[#817472]">{label}</p><p className="mt-1 font-semibold">{value}</p></div>; }
